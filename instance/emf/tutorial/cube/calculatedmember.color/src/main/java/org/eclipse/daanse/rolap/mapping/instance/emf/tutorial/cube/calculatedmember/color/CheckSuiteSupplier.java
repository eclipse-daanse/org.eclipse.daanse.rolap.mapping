...
public class CheckSuiteSupplier implements OlapCheckSuiteSupplier {
    ...
    public OlapCheckSuite get() {
        ...
        MeasureAttributeCheck measureSumAttributeCheckFormat = factory.createMeasureAttributeCheck();
        ...
        MeasureAttributeCheck measureCountAttributeCheckFormat = factory.createMeasureAttributeCheck();
        ...
        MeasureAttributeCheck calculatedMember2AttributeCheckFormat = factory.createMeasureAttributeCheck();
        calculatedMember2AttributeCheckFormat.setAttributeType(MeasureAttribute.FORMAT_STRING);
        calculatedMember2AttributeCheckFormat.setExpectedValue("$#,##;BACK_COLOR=255;FORE_COLOR=13369395");

        measureCheckCalculatedMember2.getMeasureAttributeChecks().add(calculatedMember2AttributeCheckFormat);
        ...
        queryCheck1.setQuery("SELECT [Measures].[Measure-Sum] ON COLUMNS FROM [Cube CalculatedMember with different colors]");
        ...
        queryCheck2.setQuery("SELECT [Measures].[Measure-Count] ON COLUMNS FROM [Cube CalculatedMember with different colors]");
        ...
        queryCheck3.setQuery("SELECT [theHierarchy].[Calculated Member 1] ON COLUMNS FROM [Cube CalculatedMember with different colors]");
        ...
        queryCheck4.setQuery("SELECT [Measures].[Calculated Member 2] ON COLUMNS FROM [Cube CalculatedMember with different colors]");
        ...
        catalogCheck.setName("Daanse Tutorial - Cube Calculated Member Color catalog check");
        catalogCheck.setDescription("Check that catalog 'Daanse Tutorial - Cube Calculated Member Color' exists with its cubes");
        catalogCheck.setCatalogName("Daanse Tutorial - Cube Calculated Member Color");
        ...
        connectionCheck.setName("Connection Check for Cube CalculatedMember with different colors");
        connectionCheck.setDescription("Connection check for Cube CalculatedMember with different colors");
        connectionCheck.getCatalogChecks().add(catalogCheck);

        // Create suite containing the connection check
        OlapCheckSuite suite = factory.createOlapCheckSuite();
        suite.setName("Daanse Tutorial - Cube Calculated Member Color Cube Suite");
        suite.setDescription("Check suite for the Daanse Tutorial - Cube Calculated Member Color");

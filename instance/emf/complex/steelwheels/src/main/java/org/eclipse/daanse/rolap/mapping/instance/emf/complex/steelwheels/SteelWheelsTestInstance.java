...
/**
 * Auto-discovered test instance for the SteelWheels catalog.
 * Minimal CSV data ships under {@code ./data/} (header + one dummy row per
 * table) so the schema-existence checks pass; value-level checks would
 * require richer data.
 */
public class SteelWheelsTestInstance implements CatalogTestInstance {

    @Override
    public String name() {
        return "complex.steelwheels";
    }
    ...
    public DatabaseCheckSuiteSupplier dbCheckSupplier() {
        return new SteelWheelsDatabaseCheckSuiteSupplier();
    }
    ...
    public DatabaseSupplier databaseSupplier() {
        return new SteelWheelsDatabaseSupplier();
    }

...
/**
 * Provides a check suite for the level if blank name tutorial.
 * Checks that the catalog, cube, dimension, hierarchy, levels with HideMemberIf.IF_BLANK_NAME attribute, and measure exist and are accessible.
 */
...
public class CheckSuiteSupplier implements OlapCheckSuiteSupplier {
    ...
    public OlapCheckSuite get() {
        ...
        cubeCheck.setName("CubeCheck-HiddenMembersIfBlankName");
        cubeCheck.setDescription("Check that cube 'HiddenMembersIfBlankName' exists");
        cubeCheck.setCubeName("HiddenMembersIfBlankName");
        ...
        queryCheck.setQuery("SELECT FROM [HiddenMembersIfBlankName] WHERE ([Measures].[Measure1])");
        ...
        // Create database column checks for Level_2_NULL table
        ...
        DatabaseTableCheck databaseTableCheckLevel2Null = factory.createDatabaseTableCheck();
        databaseTableCheckLevel2Null.setName("Database Table Level_2_NULL Check");
        databaseTableCheckLevel2Null.setTableName("Level_2_NULL");
        databaseTableCheckLevel2Null.getColumnChecks().add(columnCheckLevel2Key);
        databaseTableCheckLevel2Null.getColumnChecks().add(columnCheckLevel2Name);
        databaseTableCheckLevel2Null.getColumnChecks().add(columnCheckLevel2L1Key);

        // Create Database Schema Check
        DatabaseSchemaCheck databaseSchemaCheck = factory.createDatabaseSchemaCheck();
        databaseSchemaCheck.setName("Database Schema Check");
        databaseSchemaCheck.setDescription("Database Schema Check for Daanse Tutorial - Level If Blank Name");
        ...
        catalogCheck.setName("Daanse Tutorial - Level If Blank Name");
        catalogCheck.setDescription("Check that catalog 'Daanse Tutorial - Level If Blank Name' exists with HideMemberIf.IF_BLANK_NAME level");
        catalogCheck.setCatalogName("Daanse Tutorial - Level If Blank Name");
        ...
        connectionCheck.setName("Connection Check Daanse Tutorial - Level If Blank Name");
        connectionCheck.setDescription("Connection check for Daanse Tutorial - Level If Blank Name");
        connectionCheck.getCatalogChecks().add(catalogCheck);

        // Create suite containing the connection check
        OlapCheckSuite suite = factory.createOlapCheckSuite();
        suite.setName("Daanse Tutorial - Level If Blank Name");
        suite.setDescription("Check suite for the Daanse Tutorial - Level If Blank Name with HideMemberIf.IF_BLANK_NAME attribute");
